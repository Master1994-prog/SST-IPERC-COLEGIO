/// Representa un área perteneciente a una institución.
class AreaModel {
  const AreaModel({
    required this.id,
  ...
  static List<AreaModel> listaDesdeJson(dynamic data) {
    final List<dynamic> elementos = _extraerLista(data);

    return elementos
        .whereType<Map>()
        .map((Map elemento) {
          return AreaModel.fromJson(Map<String, dynamic>.from(elemento));
        })
        .where((AreaModel area) => area.id > 0 && area.nombre.trim().isNotEmpty)
        .toList();
  }
  ...
  static Map<String, dynamic> objetoDesdeJson(dynamic data) {
    ...
    final dynamic contenido =
        mapa['data'] ?? mapa['result'] ?? mapa['value'] ?? mapa['area'];
