...
import 'nuevo_tipo_equipo_proteccion_screen.dart';
import 'editar_tipo_equipo_proteccion_screen.dart';

/// Pantalla principal del catálogo Tipos de Equipo de Protección.
///
/// Permite:
///
/// - Consultar los tipos de EPP.
/// - Buscar por código, nombre o descripción.
/// - Actualizar el listado.
/// - Visualizar el detalle.
/// - Eliminar o desactivar registros.
///
/// Los formularios de registro y edición se conectarán
/// en los siguientes pasos.
class TiposEquipoProteccionScreen extends StatelessWidget {
  const TiposEquipoProteccionScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<TipoEquipoProteccionProvider>(
      create: (_) {
        final TipoEquipoProteccionProvider provider =
            TipoEquipoProteccionProvider();

        Future<void>.microtask(provider.cargarTipos);

        return provider;
      },
      child: const _TiposEquipoProteccionView(),
    );
  }
}

/// Vista interna del catálogo.
class _TiposEquipoProteccionView extends StatefulWidget {
  const _TiposEquipoProteccionView();

  @override
  State<_TiposEquipoProteccionView> createState() {
    return _TiposEquipoProteccionViewState();
  }
}

class _TiposEquipoProteccionViewState
    extends State<_TiposEquipoProteccionView> {
  ...
  /// Actualiza el catálogo desde el backend.
  Future<void> _actualizar() async {
    await context.read<TipoEquipoProteccionProvider>().cargarTipos();
  }
  ...
  Future<void> _abrirNuevoTipo() async {
    ...
    final bool? creado = await Navigator.of(context).push<bool>(
      MaterialPageRoute<bool>(
        builder: (_) {
          return ChangeNotifierProvider<TipoEquipoProteccionProvider>.value(
            value: provider,
            child: const NuevoTipoEquipoProteccionScreen(),
          );
        },
      ),
    );
  ...
  /// Abre posteriormente el formulario de edición.
  Future<void> _abrirEditarTipo(TipoEquipoProteccionModel tipo) async {
    final TipoEquipoProteccionProvider provider = context
        .read<TipoEquipoProteccionProvider>();
    ...
    final bool? actualizado = await Navigator.of(context).push<bool>(
      MaterialPageRoute<bool>(
        builder: (_) {
          return ChangeNotifierProvider<TipoEquipoProteccionProvider>.value(
            value: provider,
            child: EditarTipoEquipoProteccionScreen(tipo: tipo),
          );
        },
      ),
    );
  ...
  /// Solicita confirmación antes de eliminar o desactivar.
  Future<void> _confirmarEliminar(TipoEquipoProteccionModel tipo) async {
    final bool? confirmado = await showDialog<bool>(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          title: const Text('Eliminar tipo de EPP'),
          content: Text(
    ...
    if (eliminado) {
      messenger
        ..hideCurrentSnackBar()
        ..showSnackBar(
          const SnackBar(content: Text('Tipo de EPP eliminado correctamente.')),
        );
    ...
    messenger
      ...
      ..showSnackBar(
        SnackBar(
          ...
          content: Text(
            provider.mensajeError ?? 'No se pudo eliminar el tipo de EPP.',
          ),
        ),
      );
  }
  ...
  void _mostrarDetalle(TipoEquipoProteccionModel tipo) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<TipoEquipoProteccionProvider>(
        builder:
            (
              BuildContext context,
              TipoEquipoProteccionProvider provider,
  ...
  /// Construye el resumen superior.
  Widget _construirResumen(TipoEquipoProteccionProvider provider) {
    return Padding(
  ...
  Widget _construirBuscador(TipoEquipoProteccionProvider provider) {
    return Padding(
  ...
  Widget _construirContenido(TipoEquipoProteccionProvider provider) {
    ...
    if (provider.tieneError && !provider.tieneTipos) {
      return ListView(
        ...
        children: <Widget>[
          ...
          const Text(
            'No se pudieron cargar los tipos de EPP',
        ...
        children: <Widget>[
          ...
          Text(
            provider.textoBusqueda.isEmpty
                ? 'No hay tipos de EPP registrados'
          ...
          Text(
            provider.textoBusqueda.isEmpty
                ? 'Registra el primer tipo de equipo '
                      'de protección.'
    ...
    return ListView.separated(
      ...
      itemBuilder: (BuildContext context, int index) {
        final TipoEquipoProteccionModel tipo = provider.tiposFiltrados[index];

        return _TipoEquipoCard(
          tipo: tipo,
...
/// Tarjeta de un tipo de EPP.
class _TipoEquipoCard extends StatelessWidget {
  const _TipoEquipoCard({
    required this.tipo,
              ...
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    const SizedBox(height: 4),
                    Text(
                      tipo.codigo.trim().isEmpty ? 'Sin código' : tipo.codigo,
                      style: TextStyle(
                        color: colorScheme.primary,
                        fontWeight: FontWeight.w600,
                      ),
